    ...
    <section className="relative overflow-hidden text-white min-h-[90vh] flex items-center justify-center">
      ...
      <div className="relative z-10 container mx-auto px-6 text-center max-w-3xl">
        <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight leading-tight animate-fadeUp">
          Забайкальская{" "}
          <span className="text-[#6E0A6B] drop-shadow-lg">кино</span>комиссия
        </h1>
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-6 animate-fadeUp delay-400">
          <Link
            href="/news"
            className="px-8 py-4 rounded-xl bg-[#6E0A6B] text-white font-medium shadow-lg hover:scale-105 transition-transform"
          >
            Новости
          </Link>
          <Link
            href="/locations"
            className="px-8 py-4 rounded-xl border border-white/40 text-white font-medium hover:bg-white/10 transition"
          >
            Локации
          </Link>
        </div>
...
function Features() {
  const items = [
    { number: "250+", label: "Локаций в каталоге" },
    { number: "50+", label: "Партнёрских проектов" },
    { number: "15 лет", label: "поддержки киноиндустрии" },
  ];
  return (
    <section className="py-20 relative">
      <div className="container mx-auto px-6 grid md:grid-cols-3 gap-10 text-center">
        {items.map((s, i) => (
          ...
          >
            <div className="text-5xl font-extrabold text-[#6E0A6B]">
              {s.number}
            </div>
            <div className="mt-3 text-gray-600 dark:text-gray-300 text-lg font-medium">
              {s.label}
            </div>
          </div>
        ))}
  ...
  return (
    <section className="py-24 bg-gradient-to-br from-[#6E0A6B]/5 via-purple-200/10 to-transparent dark:from-[#6E0A6B]/20">
      <div className="container mx-auto px-6">
        <h2 className="text-4xl font-bold text-center mb-16 animate-fadeUp">
          Что мы предлагаем
        </h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10">
          {services.map((s, i) => (
            ...
            >
              ...
              <Link
                href={s.href}
                className="inline-block px-4 py-2 rounded-lg border border-[#6E0A6B] text-[#6E0A6B] font-medium hover:bg-[#6E0A6B] hover:text-white transition"
              >
                Подробнее
              </Link>
            </div>
    ...
    <section className="relative py-32 text-center text-white overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6 max-w-2xl">
        ...
        <p className="text-lg text-gray-200 mb-8 animate-fadeUp delay-200">
          Мы открыты для новых партнёров и идей. Оставьте заявку и начните
          сотрудничество.
        </p>
        <Link
          href="/contacts"
          className="px-10 py-5 rounded-xl bg-white text-[#6E0A6B] font-semibold shadow-xl hover:scale-105 transition-transform text-lg"
        >
          Связаться с нами
        </Link>
      </div>
